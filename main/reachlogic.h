...
bool reachOrientation(float current_theta, float desired_theta, PIDController orientationPID) {
    ...
    if (steering_angle > 0) {
        sendSteeringCommand(steering_angle, 'L', 0, 0);
    } else if (steering_angle < 0) {
        sendSteeringCommand(-steering_angle, 'R', 0, 0);
    } else {
        sendSteeringCommand(0, 'F', 0, 0);
    }
}
...
bool moveTowardsPoint(
    ...
    int steering_angle = (int)(orientationPID.compute(0, diff_theta));
    if (steering_angle > 0)
        sendSteeringCommand(steering_angle, 'L', speed, 1);
    else if (steering_angle < 0)
        sendSteeringCommand(-steering_angle, 'R', speed, 1);
    else
        sendSteeringCommand(0, 'F', speed, 1);
